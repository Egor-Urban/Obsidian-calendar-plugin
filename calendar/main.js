...
function getDailyNoteSettings() {
    try {
        ...
        const calendarPlugin = plugins.getPlugin("calendar");
        const customDailyFolder = calendarPlugin?.options?.dailyNoteFolder?.trim();
        
        if (shouldUsePeriodicNotesSettings("daily")) {
            const { format, folder, template } = plugins.getPlugin("periodic-notes")?.settings?.daily || {};
            return {
                format: format || DEFAULT_DAILY_NOTE_FORMAT,
                folder: customDailyFolder || folder?.trim() || "",
...
class CalendarSettingsTab extends obsidian.PluginSettingTab {
    ...
    addDailyNoteFolderSetting() {
        new obsidian.Setting(this.containerEl)
            ...
            .setDesc("Specify the folder where daily notes will be saved (optional)")
            .addText((textfield) => {
            textfield.setValue(this.plugin.options.dailyNoteFolder);
            textfield.setPlaceholder("Daily notes");
            textfield.onChange(async (value) => {
                this.plugin.writeOptions(() => ({ dailyNoteFolder: value }));
            });
        });
    }
